...
Order Supervisor is a local interview proof of concept. One durable Temporal
Workflow supervises one order, receives events and operator controls over time,
and projects its important state into PostgreSQL for the FastAPI read model and
audit timeline.

The implemented POC supports a real local `qwen3:1.7b` supervisor through
Ollama and retains the deterministic provider for tests and no-model
development. No paid API key or external messaging service is required.
...
The repository owner will manually record and review the final walkthrough video
in Stage 5. No video has been generated by Codex, and all acceptance-matrix video
timestamps remain `TBD`.
...
Stage 5 will add the short architecture note, final fresh-setup/end-to-end
validation, and recording-support artifacts. The repository owner must manually
record and review the walkthrough video; it remains not started, with all
timestamps `TBD`.

Authentication, real external messaging, cloud deployment, RAG, vector
databases, queues, multi-agent behavior, and production infrastructure remain
